...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_query_sums(self, options):
        ...
        if (cur.id != company.currency_id.id):
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                query = query.replace('SUM(ROUND(account_move_line.debit * currency_table.rate, currency_table.precision))   AS debit', '(CASE WHEN (SUM(ROUND(account_move_line.debit / ROUND((1/currency_rate.rate),4), 2)) > 0) THEN SUM(CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (ROUND(account_move_line.debit / account_move.l10n_ar_currency_rate, 2)) ELSE (ROUND(account_move_line.debit / ROUND((1/currency_rate.rate),4), 2)) END) ELSE (0.0) END) AS debit')
                query = query.replace('SUM(ROUND(account_move_line.credit * currency_table.rate, currency_table.precision))  AS credit', '(CASE WHEN (SUM(ROUND(account_move_line.credit / ROUND((1/currency_rate.rate),4), 2)) > 0) THEN SUM(CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (ROUND(account_move_line.credit / account_move.l10n_ar_currency_rate, 2)) ELSE (ROUND(account_move_line.credit / ROUND((1/currency_rate.rate),4), 2)) END) ELSE (0.0) END) AS credit')
                query = query.replace('SUM(ROUND(account_move_line.balance * currency_table.rate, currency_table.precision)) AS balance', '(CASE WHEN (SUM(ROUND(account_move_line.balance / ROUND((1/currency_rate.rate),4), 2)) > 0) THEN SUM(CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (ROUND(account_move_line.balance / account_move.l10n_ar_currency_rate, 2)) ELSE (ROUND(account_move_line.balance / ROUND((1/currency_rate.rate),4), 2)) END) ELSE (0.0) END) AS balance')
    ...
    def _get_aml_values(self, options, partner_ids, offset=0, limit=None):
        ...
        if (cur.id != company.currency_id.id):
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                query = query.replace('account_move_line.amount_currency,', 'ROUND(COALESCE((CASE WHEN account_move_line.currency_id=currency_rate.currency_id THEN (CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (ROUND(account_move_line.amount_currency * account_move.l10n_ar_currency_rate, 2)) ELSE (ROUND(account_move_line.amount_currency * ROUND((1/currency_rate.rate),4), 2)) END) ELSE account_move_line.amount_currency END) / (CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (account_move.l10n_ar_currency_rate) ELSE (ROUND((1/currency_rate.rate),4)) END), 0.0), 2) AS amount_currency,')
            ...
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                query = query.replace('account_move_line.amount_currency,', "ROUND(COALESCE((CASE WHEN account_move_line.currency_id=currency_rate.currency_id THEN (CASE WHEN (move_type = 'in_invoice') THEN (account_move_line.balance) ELSE (CASE WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 0) THEN (ROUND(account_move_line.amount_currency * account_move.l10n_ar_currency_rate, 2)) ELSE (ROUND(account_move_line.amount_currency * ROUND((1/currency_rate.rate),4), 2)) END) END) ELSE account_move_line.amount_currency END), 0.0), 2) AS amount_currency,")
                query = query.replace('account_move.name                                                                AS move_name,', 'account_move.name         AS move_name, (select count(distinct am.id) FROM account_move_line aml LEFT JOIN account_move am ON (aml.move_id = am.id) where am.name = account_move.name ) AS count_move_name,')
                query = query.replace('account_move.name                                                                   AS move_name,', 'account_move.name         AS move_name, (select count(distinct am.id) FROM account_move_line aml LEFT JOIN account_move am ON (aml.move_id = am.id) where am.name = account_move.name ) AS count_move_name,')
                query = query.replace('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")', 'FROM "account_move_line" INNER JOIN "res_company" as "company_rate" ON ("account_move_line"."company_id" = "company_rate"."id") LEFT JOIN "res_currency_rate" as "currency_rate" ON ("account_move_line"."date" = "currency_rate"."name" AND "company_rate"."foreign_currency_id" = "currency_rate"."currency_id" AND "company_rate"."id" = "currency_rate"."company_id") LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")')
                query = query.replace('ORDER BY account_move_line.date, account_move_line.id', ' AND (journal.exclude_report is NULL or journal.exclude_report = False) ORDER BY account_move_line.date, account_move_line.id')
