    ...
    def _get_query_sums(self, options):
        ...
        if (cur.id != company.currency_id.id):
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                query = query.replace('SUM(ROUND(account_move_line.debit * currency_table.rate, 4))   AS debit', '(CASE WHEN (LENGTH(CAST(SUM(ROUND(account_move_line.debit / ROUND((1/currency_table.rate),4), 6)) AS TEXT)) > 0) THEN SUM(CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL AND account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(account_move_line.debit / account_move_line.aux_inverse_currency_rate::numeric, 6)) WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.debit / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(account_move_line.debit / account_move.l10n_ar_currency_rate, 6)) ELSE (ROUND(account_move_line.debit / ROUND((1/currency_rate.rate),4), 6)) END) ELSE (0.0) END) AS debit')
                query = query.replace('SUM(ROUND(account_move_line.credit * currency_table.rate, 4))  AS credit', '(CASE WHEN (LENGTH(CAST(SUM(ROUND(account_move_line.credit / ROUND((1/currency_table.rate),4), 6)) AS TEXT)) > 0) THEN SUM(CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL AND account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(account_move_line.credit / account_move_line.aux_inverse_currency_rate::numeric, 6)) WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.credit / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(account_move_line.credit / account_move.l10n_ar_currency_rate, 6)) ELSE (ROUND(account_move_line.credit / ROUND((1/currency_rate.rate),4), 6)) END) ELSE (0.0) END) AS credit')
                query = query.replace('SUM(ROUND(account_move_line.balance * currency_table.rate, 4)) AS balance', '(CASE WHEN (LENGTH(CAST(SUM(ROUND(account_move_line.balance / ROUND((1/currency_table.rate),4), 6)) AS TEXT)) > 0) THEN SUM(CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL AND account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(account_move_line.balance / account_move_line.aux_inverse_currency_rate::numeric, 6))  WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.balance / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(account_move_line.balance / account_move.l10n_ar_currency_rate, 6)) ELSE (ROUND(account_move_line.balance / ROUND((1/currency_rate.rate),4), 6)) END) ELSE (0.0) END) AS balance')
                query = query.replace('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")', 'FROM "account_move_line" INNER JOIN "account_move" ON ("account_move"."id" = "account_move_line"."move_id") INNER JOIN "res_company" as "company_rate" ON ("account_move_line"."company_id" = "company_rate"."id") LEFT JOIN "res_currency_rate" as currency_rate ON currency_rate.id = (SELECT id FROM "res_currency_rate" rcr WHERE rcr.name <= account_move_line.date AND rcr.currency_id = company_rate.foreign_currency_id AND rcr.company_id = company_rate.id ORDER BY name DESC LIMIT 1) LEFT JOIN account_payment ON "account_move_line"."payment_id" = account_payment.id LEFT JOIN account_payment_group ON account_payment.payment_group_id = account_payment_group.id LEFT jOIN "account_account" aS "account_move_line__account_id" oN ("account_move_line"."account_id" = "account_move_line__account_id"."id") LEFT JOIN account_journal journal ON (journal.id = account_move_line.journal_id) ')
            ...
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                query = query.replace('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")', 'FROM "account_move_line" INNER JOIN "res_company" as "company_rate" ON ("account_move_line"."company_id" = "company_rate"."id") LEFT JOIN "res_currency_rate" as currency_rate ON currency_rate.id = (SELECT id FROM "res_currency_rate" rcr WHERE rcr.name <= account_move_line.date AND rcr.currency_id = company_rate.foreign_currency_id AND rcr.company_id = company_rate.id ORDER BY name DESC LIMIT 1) LEFT JOIN account_payment ON "account_move_line"."payment_id" = account_payment.id LEFT JOIN account_payment_group ON account_payment.payment_group_id = account_payment_group.id LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id") LEFT JOIN account_journal journal ON (journal.id = account_move_line.journal_id) ')
                query = query.replace('GROUP BY account_move_line.partner_id', ' AND (journal.exclude_report is NULL or journal.exclude_report = False) GROUP BY account_move_line.partner_id')
                #_logger.info('apl-_get_query_sums- cur ==')
    ...
    def _get_aml_values(self, options, partner_ids, offset=0, limit=None):
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                SELECT
                    ...
                    ROUND(account_move_line.debit * currency_table.rate, 4)   AS debit,
                    ROUND(account_move_line.credit * currency_table.rate, 4)  AS credit,
                    ROUND(account_move_line.balance * currency_table.rate, 4) AS balance,
            ...
            queries.append(f'''
                SELECT
                    ...
                    CASE WHEN aml_with_partner.balance > 0 THEN 0 ELSE ROUND(
                        partial.amount * currency_table.rate, 4
                    ) END                                                                               AS debit,
                    CASE WHEN aml_with_partner.balance < 0 THEN 0 ELSE ROUND(
                        partial.amount * currency_table.rate, 4
                    ) END                                                                               AS credit,
        ...
        if (cur.id != company.currency_id.id):
            if (query.find('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")') > 0):
                ...
                query = query.replace('account_move.name                                                                   AS move_name,', 'account_move.name         AS move_name, (select count(distinct am.id) FROM account_move_line aml LEFT JOIN account_move am ON (aml.move_id = am.id) where am.name = account_move.name AND aml.partner_id = "account_move_line"."partner_id") AS count_move_name,')
                query = query.replace('ROUND(account_move_line.debit * currency_table.rate, 4)   AS debit,','(CASE WHEN (account_move_line.debit <> 0) THEN (CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL and account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.debit / account_move_line.aux_inverse_currency_rate::numeric), 6) * currency_table.rate, 4)) WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.debit / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.debit / account_move.l10n_ar_currency_rate), 6) * currency_table.rate, 4)) WHEN (currency_rate.rate > 0) THEN (ROUND(ROUND((account_move_line.debit / ROUND((1/currency_rate.rate),4)), 6) * currency_table.rate, 4)) ELSE (0.0) END) ELSE (0.0) END) AS debit,')
                query = query.replace('ROUND(account_move_line.credit * currency_table.rate, 4)  AS credit,','(CASE WHEN (account_move_line.credit <> 0) THEN (CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL and account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.credit / account_move_line.aux_inverse_currency_rate::numeric), 6) * currency_table.rate, 4)) WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.credit / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.credit / account_move.l10n_ar_currency_rate), 6) * currency_table.rate, 4)) WHEN (currency_rate.rate > 0) THEN (ROUND(ROUND((account_move_line.credit / ROUND((1/currency_rate.rate),4)), 6) * currency_table.rate, 4)) ELSE (0.0) END) ELSE (0.0) END) AS credit,')
                query = query.replace('ROUND(account_move_line.balance * currency_table.rate, 4) AS balance,','(CASE WHEN (account_move_line.balance <> 0) THEN (CASE WHEN (account_move_line.aux_inverse_currency_rate IS NOT NULL and account_move_line.aux_inverse_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.balance / account_move_line.aux_inverse_currency_rate::numeric), 6) * currency_table.rate, 4)) WHEN (account_payment_group.exchange_rate_applied IS NOT NULL and account_payment_group.exchange_rate_applied > 1) THEN (ROUND(ROUND((account_move_line.balance / account_payment_group.exchange_rate_applied::numeric), 6) * currency_table.rate, 4)) WHEN (account_move.l10n_ar_currency_rate IS NOT NULL and account_move.l10n_ar_currency_rate > 1) THEN (ROUND(ROUND((account_move_line.balance / account_move.l10n_ar_currency_rate), 6) * currency_table.rate, 4)) WHEN (currency_rate.rate > 0) THEN (ROUND(ROUND((account_move_line.balance / ROUND((1/currency_rate.rate),4)), 6) * currency_table.rate, 4)) ELSE (0.0) END) ELSE (0.0) END) AS balance,')
                query = query.replace('FROM "account_move_line" LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id")', 'FROM "account_move_line" INNER JOIN "res_currency" ON ("account_move_line"."currency_id" = "res_currency"."id") INNER JOIN "res_company" as "company_rate" ON ("account_move_line"."company_id" = "company_rate"."id") LEFT JOIN "res_currency_rate" as currency_rate ON currency_rate.id = (SELECT id FROM "res_currency_rate" rcr WHERE rcr.name <= "account_move_line"."date" AND rcr.currency_id = "company_rate"."foreign_currency_id" AND rcr.company_id = "company_rate"."id" ORDER BY rcr.name DESC LIMIT 1) LEFT JOIN account_payment ON "account_move_line"."payment_id" = account_payment.id LEFT JOIN account_payment_group ON account_payment.payment_group_id = account_payment_group.id LEFT JOIN "account_account" AS "account_move_line__account_id" ON ("account_move_line"."account_id" = "account_move_line__account_id"."id") ')
